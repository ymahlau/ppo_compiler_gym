...
class llvm_wrapper(gym.Env):
    ...
    """
    def __init__(self, benchmarks, max_episode_steps = None, steps_in_observation = False, patience = None, allowed_actions = None):
        ...
        if self.limited_time:
            ...
            if steps_in_observation:
                self.observation_space = gym.spaces.Box(0,9223372036854775807,np.shape(list(range(1+self.env.observation_space.shape[0]))),dtype=np.int64)
            else:
        ...
        if self.limited_action_set:
            ...
            self.action_space = gym.spaces.Discrete(self.env.action_space.n if allowed_actions is None else len(allowed_actions))
        else:
